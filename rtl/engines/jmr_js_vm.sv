    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            // unused-FF. Plain case sequences; SRAM still rdata<=mem[raddr].
                ...
                S_JSON_PARSE: begin
                    if (json_rp >= json_src + json_srclen) begin
                        stack_wr(json_res, (js_sp == 0) ? 32'sd0 : js_val[0], (js_sp == 0) ? 3'd5 : js_tag[0]);
                        sp <= json_res + 11'd1;
                        hs_code(15'(ops_base + ip));
                        hs_st(S_FETCH_WAIT);
                    end else if (!vjs_rd_arm) begin
            ...
            // flatten: heap write via S_HEAP_* only
                            ...
                            json_rp <= json_rp + 14'd1;
                            if (js_sp <= 6'd1) begin
                                stack_wr(json_res, js_val[0], 3'd2);
                                sp <= json_res + 11'd1;
                                hs_code(15'(ops_base + ip));
                                hs_st(S_FETCH_WAIT);
                            end else begin
            ...
            // flatten: heap write via S_HEAP_* only
                            ...
                            json_rp <= json_rp + 14'd1;
                            if (js_sp <= 6'd1) begin
                                stack_wr(json_res, js_val[0], 3'd1);
                                sp <= json_res + 11'd1;
                                hs_code(15'(ops_base + ip));
                                hs_st(S_FETCH_WAIT);
                        ...
                        if (ch == 8'h20 || ch == 8'h0A ||
                            ...
                            if (!vfree_armed) begin
                                ...
                                if (!vfree_ok) begin
                                    ...
                                    hs_valloc_retried(1'b0);
                                    varr_valid[valloc_i[11:0]] <= 1'b1;
                                    varr_len_wr(valloc_i[11:0], 8'd0);
